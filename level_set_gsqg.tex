...
Here the notation ``$C$'' refers to the space of \emph{all} continuous functions,
not necessarily bounded.\smallskip
...
\[
    ...
    \omega^{t}(x)\,dx
\]
holds for all $\varphi\in C_{c}^{1}(\bbR^{2})$.\bigskip
...
(i)    Then $\Phi_{*}^{t}\Omega$ is composed of simple closed curves for each $t\in I$, and
  ...
  (iii) If $\alpha\in(0, \frac 16]$ and $T$ is a common endpoint of $I$ and $I'$,
  then either $|T|=\infty$ or $\limsup_{t\to T} Q(\Phi_{*}^{t}\Omega) = \infty$.
\end{theorem}
...
\[
    Q(\Phi_{\eps,N*}^{t+h}\Omega_{N}) \leq e^{C\abs{h}}Q(\Phi_{\eps,N*}^{t}\Omega_{N})
\]
for the same $(t,h,\lambda)$, with a new $C$ depending again only on $\alpha,\eps,N,\Omega,\tht$, 
from which we conclude the  $Q(\Phi_{\eps,N*}^{t}\Omega_{N})$ is upper semi-continuous in $t\in[-T_0,T_0]$ for all $N,\eps$ (this then clearly extends to all $t\in\bbR$).
...
(the latter by \cite[Corollary~B.3]{JeoZlaTouching}) and \eqref{3.14} show that
\[
    \ell(z_{\eps}^{t,\lambda})\norm{z_{\eps}^{t,\lambda}}_{\dot{H}^{2}}^{2}
