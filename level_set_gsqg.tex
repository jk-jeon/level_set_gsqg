...
for some $\beta\in(0,1]$, then $R_{\mu}(\Theta)<\infty$ if and only if $\beta>\frac{1}{2}$.
...
Fix any $\eps>0$. We continue to use the notations
$\Phi_{\eps}^{t}$, $\theta_{\eps}^{t}$,
$z_{\eps}^{t,\lambda}$, $\ell_{\eps}^{t,\lambda}$,
$\mathbf{T}_{\eps}^{t,\lambda}(s)$, $\mathbf{N}_{\eps}^{t,\lambda}(s)$,
and $u_{\eps}^{t,\lambda}(s)$ from Section~\ref{S3}
(with the chosen arclength parametrization of $z_{\eps}^{t,\lambda}$).

\begin{lemma}
    \[
        \norm{\partial_{s}u_{\eps}^{t,\lambda}}_{L^{\infty}}
    \]
\end{lemma}

\begin{proof}
    In this proof, we suppress $t$ from the notations for simplicity
    since we will only work with quantities at a single time instance.
    For any $s\in\ell_{\eps}^{\lambda}\bbT$, Fubini's theorem
    (recall $\Theta$ is of finite measure) and Green's theorem show
    \begin{align*}
        \partial_{s}u_{\eps}^{\lambda}(s)
        &= D\big(u_{\eps}(\theta_{\eps})\big)(z_{\eps}^{\lambda}(s))
        (\mathbf{T}_{\eps}^{\lambda}(s)) \\
        &= \int_{\bbR^{2}}D(\nabla^{\perp}K_{\eps})
        (z_{\eps}^{\lambda}(s) - y)(\mathbf{T}_{\eps}^{\lambda}(s))
        \theta_{\eps}(y)\,dy \\
        &= \int_{\mathcal{L}}\int_{\Phi_{\eps}(\Theta^{\lambda'})}
        D(\nabla^{\perp}K_{\eps})
        (z_{\eps}^{\lambda}(s) - y)(\mathbf{T}_{\eps}^{\lambda}(s))\,dy\,d\mu(\lambda') \\
        &= \int_{\mathcal{L}}\int_{\ell_{\eps}^{\lambda'}\bbT}
        DK_{\eps}(z_{\eps}^{\lambda}(s) - z_{\eps}^{\lambda'}(s'))
        (\mathbf{T}_{\eps}^{\lambda}(s))\mathbf{T}_{\eps}^{\lambda'}(s')\,ds'\,d\mu(\lambda').
    \end{align*}
    Then the proof of (2.3) in \cite{JeoZla2} (with $\beta=1/2$) applies and we obtain
    that the integrand is bounded by
    \[
        \ell_{\eps}^{\lambda'}\max\set{
            \norm{z_{\eps}^{\lambda}}_{\dot{H}^{2}},
            \norm{z_{\eps}^{\lambda'}}_{\dot{H}^{2}}
        }^{2+4\alpha}
        \leq \ell_{\eps}^{\lambda'}\left(
            \frac{Q(\Phi_{\eps*}\Theta)}{\Sigma(\Phi_{\eps*}\Theta)}
        \right)^{1+2\alpha}.
    \]
\end{proof}
