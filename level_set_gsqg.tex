...
$\left(0,\norm{\omega^{+}}_{L^{\infty}}\right]$ and
$\left[-\norm{\omega^{-}}_{L^{\infty}},0\right)$, so that \eqref{1.3}
...
\begin{theorem}\label{T1.2}
    ...
    $L_{\tht}(\Omega)<\infty$. Then there is a Lagrangian solution
    $\omega\colon I\to L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$ to \eqref{1.1}--\eqref{1.2}
    on some open interval $I\ni 0$, which we choose to be maximal possible,
    ...
    Such a solution is unique and independent of the choice of $(\Omega,\theta)$.
...
\begin{proof}
    Let $d\coloneqq\norm{\Phi_{1} - \Phi_{2}}_{L^{\infty}}$,
    $\mathcal{L}_{i}$ the measurable space associated to $(\Omega_{i},\theta_{i})$ and
    $\omega_{i}\coloneqq \omega\circ\Phi_{i}^{-1}$ for $i=1,2$, and fix any $x\in\bbR^{2}$.
    Then $u(\Phi_{1*}\omega;x) - u(\Phi_{2*}\omega;x)$ is the sum of the integrals
    \begin{align*}
...
\begin{proposition}\label{P2.9}
    ...
    each $\Phi_{i}^{t}$ is a measure-preserving homeomorphism and
    $\sup_{t\in I}\max_{i=1,2}L_{\theta_{i}}(\Phi_{i*}^{t}\Omega_{i}) <\infty$.
    Then $\Phi_{1} = \Phi_{2}$.
\end{proposition}
...
\begin{proof}
    ...
    $\norm{\Phi_{1}^{t}-\Phi_{2}^{t}}_{L^{\infty}}$ in $t$ shows
    \[
        \max\set{
            \partial_{t}^{+}\norm{\Phi_{1}^{t} - \Phi_{2}^{t}}_{L^{\infty}},
            -\partial_{t-}\norm{\Phi_{1}^{t} - \Phi_{2}^{t}}_{L^{\infty}}
...
Combining Propositions \ref{P2.8} and \ref{P2.9} now shows all claims of Theorem~\ref{T1.2},
except that the time spans of maximal solutions for
different generalized layer cake representations of $\omega^{0}$ must coincide to each other.
This remaining claim then follows from \eqref{1.7} and that
$\sup_{t\in J}\norm{(\Phi^{t})^{-1}}_{\dot{C}^{0,1}} < \infty$
for any compact subinterval $J$ of any of the two time spans.
