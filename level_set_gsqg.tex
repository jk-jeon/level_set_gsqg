...
%    All constants $C_{\alpha}$ in this proof depend only on $\alpha$ and
    ...
    \begin{align*}
        G_{1} &=
        \int_{\ell_{\eps}^{\lambda}\bbT}\int_{\mathcal{L}}\int_{\bbR^{2}}
        ...
        \,dy\,d\theta(\lambda')\,ds\\
        &= \int_{\mathcal{L}}\int_{\ell_{\eps}^{\lambda}\bbT}\int_{\bbR^{2}}
        \kappa_{\eps}^{\lambda}(s)\mathbf{N}_{\eps}^{\lambda}(s) \cdot
        D^{2}(\nabla^{\perp}K_{\eps})(z_{\eps}^{\lambda}(s) - y)
        (\mathbf{T}_{\eps}^{\lambda}(s), \mathbf{T}_{\eps}^{\lambda}(s))
        \mathbbm{1}_{\Phi_{\eps}(\Omega^{\lambda'})}(y)
        ...
        \,ds'\,ds\,d\theta(\lambda').
    \end{align*}
    (Note that we do not know joint measurability of the inner integrand
    of the right-hand side in the last equality, because the parametrizations
    $z_{\eps}^{\lambda'}(\cdot)$ are chosen arbitrarily and independently from each other;
    we only know the measurability with respect to $\lambda'$ of the inner integral.)
    From
    \[
    ...
    Here $\overline{\int_{\mathcal{L}}}f(\lambda')\,d|\theta|(\lambda')$
    for  $f\colon\mathcal{L}\to[0,\infty]$ is the \emph{upper Lebesgue integral}
    \[
        \overline{\int_{\mathcal{L}}}f(\lambda')\,d|\theta|(\lambda')
        \coloneqq \inf_{g}\int_{\mathcal{L}}g(\lambda')\,d|\theta|(\lambda'),
    \]
