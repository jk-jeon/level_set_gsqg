...
%    \end{gathered}
    \end{equation*}
    We also denote $\ell(\partial\Theta^{\lambda}):=\ell(z^{\lambda})$.
\end{definition}
...
{\it Remark.}  Note that the last hypothesis in (iii) excludes initial data that are bumps with rounded tops/bottoms (instead, the latter must be flat). As mentioned above,  this result means that any finite time singularity development for the corresponding solutions must be accompanied by a loss of $H^2$-regularity of  level sets, not just their collisions or pile ups.
%, and in particular cannot be caused by a ``pile up'' or collision of level sets while these remain regular.
