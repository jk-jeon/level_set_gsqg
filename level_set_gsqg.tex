...
\smallskip
\textit{Remark}. For some $t_{0}$, we will need to consider \eqref{3.1} with
$\omega_{\eps}$ and $\tilde{z}_{\eps}$ resulting from replacing
$z^{0}$ by $z^{t_{0}}$ and $\Psi_{\eps}^{t}$
by the solution to \eqref{2.1} with $\omega^{0}$ replaced by $\omega^{t_{0}}$
and with the initial time at $t = t_{0}$.
With those, we do not yet know if \eqref{3.1} holds in $H^{2}(\bbT;\bbR^{2})$
since we have not yet shown that each $z^{t_{0},\lambda}$ is an $H^{2}$ curve.
Nevertheless, since $\Phi^{t_{0}}$ is Lipschitz, we can easily see that
\eqref{3.1} still holds in $C^{0,1}(\bbT;\bbR^{2})$ in that setting and
...
\begin{proof}
   Since $\ell\colon\operatorname{CC}(\bbR^{2})\to[0,\infty]$ is lower semicontinuous
    ...
    Repeating the proof of \eqref{3.4} with the setting described in
    the remark after \eqref{3.2} shows that
...
%    With $T_{0}$ as in Proposition~\ref{P2.8},
    ...
    with $\Phi$ in place of $\Phi_{\eps}$. Then by the remark after \eqref{3.1},
    applying the same argument to $(\omega^{t},t)$ in place of $(\omega^{0},0)$ shows that
...
%    All constants $C_{\alpha}$ in this proof depend only on $\alpha$ and
    ...
    \begin{align*}
        G_{1} &= -\int_{\mathcal{L}}\int_{\ell_{\eps}^{\lambda}\bbT\times\ell_{\eps}^{\lambda'}\bbT}
