...
Note that since \eqref{1.1} is a transport PDE, it can be solved in low regularity spaces  weakly as well as in the Lagrangian sense.  The latter notion of solutions typically also guarantees the former, and we will employ it here, too (see Definition \ref{D.11.1} below).

Although $u(\theta)$ is clearly a $(1-2\alpha)$-H\"{o}lder continuous vector field when $\theta\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$, this is not sufficient to obtain well-posedness for the PDE in $L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$.  However, if also $\theta\in C^{\beta}(\bbR^{2})$ for some $\beta>2\alpha$, then $u(\theta)$ is Lipschitz and this easily yields well-posedness in $L^{1}(\bbR^{2})\cap  C^{\beta}(\bbR^{2})$ for any  $\beta>2\alpha$  \cite{}.  

On the other hand, \eqref{1.1}-\eqref{1.2} is also locally well-posed in the space of (spatially discontinuous) {\it patch solutions}
\[
\tht (t,\cdot)=\sum_{n=1}^N \tht_n \chi_{\Omega_n^t}
\]
with some constants $\tht_n\in\bbR\setminus\{0\}$ and bounded {\it patches} $\Omega_n^t\subseteq\bbR^2$ that are transported by the velocity $u(\theta(t,\cdot))$, provided all the initial patch boundaries $\partial \Omega_n^0$ are disjoint and sufficiently smooth simple closed curves \cite{}.  The reason for this is that the dynamic of these solutions is fully determined by $u(\theta(t,\cdot)))$ restricted to the patch boundaries, and as long as these remain smooth enough, the same is true for the velocity restricted to them (because near each patch boundary, $\tht(t,\cdot)$ is constant in the direction tangential to it).
...
More generally, when $\theta$ is a finite signed Borel measure on $\bbR^{2}$,
we have
\[
    u(\theta;x) \coloneqq \int_{\bbR^{2}}
    \nabla^{\perp}K(x - y)\,d\theta(y)
\]
...
It turns out that each $\Theta^{\lambda}$ being a super-level set of $\theta$
...
that encompasses situations where some level sets of $\theta$
...
\begin{equation}\label{1.100}
    ...
    \}
\end{equation}
in $(x,\lambda)$, so $L_{\mu}(\Theta)$ is always well-defined
(with the convention $\frac{1}{0} = \infty$ and $\infty\cdot 0 = 0$).
The quantity $L_{\mu}(\Theta)$ arises in the context of estimating the growth of
$H^{2}$ norm of a boundary curve $\partial\Theta^{\lambda}$ (Lemma~\ref{L3.7}),
...
\begin{definition} \lb{D.11.1}
    Let $\theta^{0}\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$.
    A \emph{Lagrangian solution}
    to \eqref{1.1}--\eqref{1.2} on a time interval $I\owns 0$ with
    the initial data $\theta^{0}$ is a function
    $\theta\colon I\to L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$ given as
    $\theta^{t}\coloneqq \theta\circ(\Phi^{t})^{-1}$, where
    ...
    We call $\Phi$ the \emph{flow map} associdated to $\theta$.
\end{definition}

Here the notation ``$C$'' refers to the space of \emph{all} continuous functions,
not necessarily bounded.\smallskip

\textit{Remark}. It is easy to show that any Lagrangian solution $\theta$ is
...
\begin{theorem}\label{T1.2}
    Let $\theta^{0}\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$ admit
    a generalized layer cake representation $(\Theta,\mu)$ such that
    $L_{\mu}(\Theta)<\infty$. Then there is an open interval $I\ni 0$ and a Lagrangian solution
    $\theta\colon I\to L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$ to \eqref{1.1}--\eqref{1.2}
    with initial data $\theta^{0}$ and
...
if each $\partial\Theta^{\lambda}$ is an $H^{2}$ curve
...
\begin{definition}
    Let $\theta\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$.
    Then a generalized layer cake representation $(\Theta,\mu)$
    of $\theta$ is said to be \emph{composed of simple closed curves} if for each
    $\lambda\in\mathcal{L}$, $\Theta^{\lambda}$ is a bounded open set and
    $\partial\Theta^{\lambda} = \operatorname{im}(z^{\lambda})$
...
since $\lambda'\mapsto\partial\Theta^{\lambda'}$ is measurable with respect to the
...
$\ell(z^{\lambda'})$ equals the perimeter of $\Theta^{\lambda'}$.
...
For our second result, we will impose in addition to $L_{\mu}(\Theta)<\infty$
that $\Theta$ is composed of simple closed curves and
$Q(\Theta), R_{\mu}(\Theta) < \infty$. The condition $Q(\Theta)<\infty$ ensures
...
The condition $R_{\mu}(\Theta)<\infty$ on the other hand controls how densely
...
near a large $z^{\lambda}$. Interpreting $z^{\lambda}$'s as level sets of $\theta$,
...
(i)    Then $\Phi_{*}^{t}\Theta$ is composed of simple closed curves for each $t\in I$, and
  ...
  then either $|T|=\infty$ or $\limsup_{t\to T} Q(\Phi_{*}^{t}\Theta) = \infty$.
\end{theorem}
...
\[
    u_{\eps}(\theta;x)\coloneqq
    \int_{\bbR^{2}}\nabla^{\perp}K_{\eps}(x-y)\,d\theta(y)
\]
for $x\in\bbR^{2}$. Since $\nabla^{\perp}K_{\eps}$ is a smooth function
whose all derivatives vanish at infinity,
this integral is always well-defined and $u_{\eps}(\theta)$ is a smooth function such that
...
We start with some estimates on the velocity field $u(\theta)$
in terms of $L_{\mu}(\Theta)$, where $(\Theta,\mu)$ is
a generalized layer cake representation of $\theta$.
...
Now, fix any initial data $\theta^{0}\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$
...
$u_{\eps}$ that is in turn generated by the measure $\Phi_{\eps *}^{t}\theta^{0}$. We will next show that \eqref{2.1} is globally well-posed in $BC(\bbR^{2};\bbR^{2})$.
...
We will next show that $\Phi_{\eps}^{t}$ is in fact a measure-preserving homeomorphism, which will mean that $\theta_{\eps}^{t}$ is an $L^{1}\cap L^\infty$ function.
...
%    All constants $C_{\alpha}$ in this proof depend only on $\alpha$ and
    ...
    $Q(\Phi_{\eps*}^{t}\Theta)\geq 4$, which in turn follows from \cite[Lemma~A.1]{JeoZlaTouching}.
 ...
 %       \mathbbm{1}_{\Phi_{\eps}(\Theta^{\lambda'})}(y)
    ...
    for $|\mu|$-almost all $\lambda'$.  Thus we can apply \cite[Lemma~A.4]{JeoZlaTouching}
    to conclude that
