...
(i)    Then $\Phi_{*}^{t}\Theta$ is composed of simple closed curves for each $t\in I$, and
    ...
    for each compact interval    $J\subseteq I'$.  Let $I'$ be the maximal such interval.  If $T$ is its endpoint,
  ...
  (iii) Suppose in addition that $\alpha\in\left(0,\frac{1}{6}\right]$,
  $\norm{\mu}_{\mathrm{TV}}<\infty$, $\Sigma(\Theta)>0$ and $\Lambda(\Theta)<\infty$.
  % If $\alpha\in(0, \frac 16]$ and $T$ is an endpoint of  $I'$,
  If $T$ is an endpoint of $I'$,
...
We now suppose $\alpha\in\left(0,\frac{1}{6}\right]$,
$\norm{\mu}_{\mathrm{TV}}<\infty$, $\Sigma(\Theta)>0$ and $\Lambda(\Theta)<\infty$.
Let $T$ be an endpoint of the interval $I'$
(recall that $I'\subseteq I$ is the maximal open interval such that
$\sup_{t\in J}Q(\Phi_{*}^{t}\Theta) < \infty$ holds for each compact interval $J\subseteq I'$),
and assume $T$ is finite. We now suppose
$\limsup_{t\to T}Q(\Phi_{*}^{t}\Theta) < \infty$ and draw a contradiction.

Note that $Q(\Phi_{*}^{t}\Theta)$ is lower semi-continuous in $t$,
...
\cite[Corollary B.3]{JeoZla2}). Therefore, $Q(\Phi_{*}^{T}\Theta)<\infty$,
so $T$ must be an endpoint of $I$ as well, because otherwise
...
Hence, it is enough to show that $\lim_{t\to T}L_{\mu}(\Phi_{*}^{t}\Theta) < \infty$,
which contradicts to Theorem~\ref{T1.2}.

We will now assume $T>0$ because the other case can be done in the same way.
Then we have $\sup_{t\in[0,T)}Q(\Phi_{*}^{t}\Theta) < \infty$,
so the isoperimetric inequality shows
\[
    ...
    \leq \sup_{t\in[0,T)}\frac{Q(\Phi_{*}^{t}\Theta)}{2\sqrt{\pi}\Sigma(\Phi_{*}^{t}\Theta)}
    = \sup_{t\in[0,T)}\frac{Q(\Phi_{*}^{t}\Theta)}{2\sqrt{\pi}\Sigma(\Theta)} < \infty.
\]
...
\begin{proof}
    ...
    \[
        ...
        \norm{\gamma}_{\dot{H}^{2}}^{2}.
    \]
    Then it is enough to prove that for any $t\in[0,T)$,
    \begin{equation}\label{4.2}
        \partial_{t}^{+}\operatorname{diam}(\operatorname{im}(z^{t,\lambda}))
        \leq C_{\alpha}\left(\norm{\theta^{0}}_{L^{1}} + \norm{\theta^{0}}_{L^{\infty}}\right),
    \end{equation}
    because then \eqref{4.1} follows by a Gr\"{o}nwall-type argument
    and the inequality $\operatorname{diam}(\operatorname{im}(\gamma)) \leq \ell(\gamma)/2$.

    Let $0\leq t\leq t+h<T$ and pick any $x,y\in\operatorname{im}(z^{0,\lambda})$
    such that $\abs{\Phi^{t+h}(x) - \Phi^{t+h}(y)}
    = \operatorname{diam}(\operatorname{im}(z^{t+h,\lambda}))$. Then
    \begin{equation}\label{4.3}\begin{aligned}
        ...
        &\leq \int_{t}^{t+h}\abs{u(\theta^{\tau};\Phi^{\tau}(x))}
        + \abs{u(\theta^{\tau};\Phi^{\tau}(y))}d\tau.
    \end{aligned}\end{equation}
    For any $\tau\in[0,T)$, we know
    \begin{equation*}
    ...
    which together with \eqref{4.3} prove \eqref{4.2}.
\end{proof}

Fix $\eta>0$ and for each $\tilde{\Theta}\subseteq\bbR^{2}\times\mathcal{L}$
in the product $\sigma$-algebra define
\[
    L_{\mu}^{\eta}(\tilde{\Theta}) \coloneqq
    \sup_{x\in\bbR^{2}}\int_{\mathcal{L}}\frac{d|\mu|(\lambda)}
    {\left(d(x,\partial\tilde{\Theta}^{\lambda}) + \eta\right)^{2\alpha}}.
\]
Then a similar proof as Proposition~\ref{P2.6} shows that
$L_{\mu}^{\eta}(\Phi_{*}^{t}\Theta)$ is continuous in $t$.
Note that $L_{\mu}^{\eta}(\tilde{\Theta}) \to L_{\mu}(\tilde{\Theta})$ as $\eta\to 0^{+}$,
so it is enough to establish an $\eta$-independent upper bound of
$L_{\mu}^{\eta}(\Phi_{*}^{t}\Theta)$.

\begin{lemma}\label{L4.2}
    There is $C_{\alpha}$ such that for any $t\in[0,T)$,
    \begin{equation}\label{4.4}
        \partial_{t}^{+}L_{\mu}^{\eta}(\Phi_{*}^{t}\Theta)
        \leq C_{\alpha}\norm{\mu}_{\mathrm{TV}}M^{2+2\alpha}\left(
            \Lambda(\Theta) + T\left(
                \norm{\theta^{0}}_{L^{1}} + \norm{\theta^{0}}_{L^{\infty}}
            \right)
        \right)
...
\begin{proof}
    Fix $t\in[0,T)$ and find a decreasing sequence $\seq{t_{n}}_{n=1}^{\infty}$
    such that $t_{n}\to t$ and
    \[
        \partial_{t}^{+}L_{\mu}^{\eta}(\Phi_{*}^{t}\Theta)
        = \lim_{n\to\infty}\frac{L_{\mu}^{\eta}(\Phi_{*}^{t_{n}}\Theta)
        - L_{\mu}^{\eta}(\Phi_{*}^{t}\Theta)}{t_{n} - t}.
    \]
    Without loss, we can assume $t_{n}\leq t + \frac{1}{2C_{\alpha}L_{\mu}(\Phi_{*}^{t}\Theta)}$
    with $C_{\alpha}$ from Lemma~\ref{L2.1}, so that
    \begin{equation}\label{4.5}
        L_{\mu}(\Phi_{*}^{\tau}\theta)\leq 2L_{\mu}(\Phi_{*}^{t}\Theta)
    \end{equation}
    holds for all $\tau\in [t,t_{n}]$, by Proposition~\ref{P2.8}.
    Fix $n\in\bbN$ and pick $x\in\bbR^{2}$ such that
    \[
        ...
        + \frac{t_{n} - t}{n}.
    \]
    Then, fix $\lambda\in\mathcal{L}$ and pick
    $y\in\operatorname{im}(z^{0,\lambda})$ such that
    \[
    ...
    Now, for given $n\in\bbN$, $L_{\mu}^{\eta}(\Phi_{*}^{t_{n}}\Theta)
    \leq L_{\mu}(\Phi_{*}^{t_{n}}\Theta) <\infty$ shows that
    for $|\mu|$-almost every $\lambda$, we have
    $\abs{\Phi^{t_{n}}(x) - \Phi^{t_{n}}(y)} > 0$,
    that is, $x\neq y$. For such $\lambda$ and any $\eta>0$,
    a similar argument as in \eqref{2.4} yields
    \begin{align*}
        &\frac{1}{\left(
            d(\Phi^{t_{n}}(x), \operatorname{im}(z^{t_{n},\lambda})) + \eta
        \right)^{2\alpha}}
    ...
    which we estimate separately:
    \begin{align*}
        P_{1} &\coloneqq \frac{\Phi^{t_{n}}(x) - \Phi^{t_{n}}(y)}
    ...
    \textbf{Estimate for $P_{1}$.} Since $\norm{\mu}_{\mathrm{TV}}<\infty$ and
    $\nabla^{\perp}K(\Phi^{t_{n}}(y) - \,\cdot\,)
    - \nabla^{\perp}K(\Phi^{t_{n}}(x) - \,\cdot\,)$ is integrable,
    Fubini's theorem and Green's theorem show
    \begin{align*}
        &u(\theta^{t_{n}};\Phi^{t_{n}}(y))
        - u(\theta^{t_{n}};\Phi^{t_{n}}(x)) \\
    ...
    By definition of $y$, $\Phi^{t_{n}}(x) - \Phi^{t_{n}}(y)$
    is normal to $z^{t_{n},\lambda}$ at $\Phi^{t_{n}}(y)$.
    Also, since $R_{\mu}(\Phi_{*}^{t_{n}}\Theta)<\infty$,
    we know that $\operatorname{im}(z^{t_{n},\lambda})
    \cap \operatorname{im}(z^{t_{n},\lambda'}) = \emptyset$ for
    $\abs{\mu}$-almost every $\lambda'$. Therefore, for such $\lambda'$,
    \cite[Lemma 2.7]{JeoZla2} (with $\beta\coloneqq 1/2$,
    $\gamma_{1}\coloneqq z^{t_{n},\lambda}$, $\gamma_{2}\coloneqq z^{t_{n},\lambda'}$)
    ...
    \textbf{Estimate for $P_{2}$.} By the same argument as in \eqref{3.6}, we have
    \[
    ...
    \begin{align*}
        ...
        \right),
    \end{align*}
    which together with Lemma~\ref{L2.1} and \eqref{4.5} yield
    \begin{equation}\label{4.7}\begin{aligned}
        \abs{P_{2}} &\leq \frac{2\left(
            e^{2C_{\alpha}L_{\mu}(\Phi_{*}^{t}\Theta)(t_{n}-t)} - 1
        \right)\norm{u(\theta^{t_{n}})}_{\dot{C}^{0,1}}(t_{n} - t)}
        {\left(
            d(\Phi^{t}(x), \operatorname{im}(z^{t,\lambda})) + \eta
        \right)^{2\alpha}} \\
        &\leq \frac{C_{\alpha}L_{\mu}(\Phi_{*}^{t}\Theta)\left(
            e^{2C_{\alpha}L_{\mu}(\Phi_{*}^{t}\Theta)(t_{n}-t)} - 1
        \right)(t_{n} - t)}
        {\left(
            d(\Phi^{t}(x), \operatorname{im}(z^{t,\lambda})) + \eta
    ...
    \textbf{Estimate for $P_{3}$.} By Lemma~\ref{L2.1}, Lemma~\ref{L2.2} and \eqref{4.5},
    \begin{align*}
        &\int_{t}^{t_{n}}
        \abs{
            u(\theta^{\tau};\Phi^{\tau}(y))
    ...
    \begin{equation}\label{4.8}
        \abs{P_{3}} \leq \frac{C_{\alpha}L_{\mu}(\Phi_{*}^{t}\Theta)
        \int_{t}^{t_{n}}\norm{\Phi^{\tau} - \Phi^{t_{n}}}_{L^{\infty}}\,d\tau}
        {\eta\left(
            d(\Phi^{t}(x),\operatorname{im}(z^{t,\lambda})) + \eta
        \right)^{2\alpha}}.
    ...
    Therefore, aggregating \eqref{4.6}, \eqref{4.7} and \eqref{4.8},
    integrating by $\lambda$, dividing by $t_{n} - t$ and then
    letting $n\to\infty$ thus shows \eqref{4.4}.
\end{proof}

Now, Lemma~\ref{L4.2} and a Gr\"{o}nwall-type argument prove for any $t\in[0,T)$,
\[
    L_{\mu}^{\eta}(\Phi_{*}^{t}\Theta)
...
with
\[
    M_{1} \coloneqq C_{\alpha}\norm{\mu}_{\mathrm{TV}}M^{2+2\alpha}\left(
        \Lambda(\Theta) + T\left(
            \norm{\theta^{0}}_{L^{1}} + \norm{\theta^{0}}_{L^{\infty}}
        \right)
    \right)
\]
for $C_{\alpha}$ from Lemma~\ref{L4.2}. Therefore, letting $\eta\to 0^{+}$
shows that
\[
...
This finishes the proof of Theorem~\ref{T1.4}(iii).
