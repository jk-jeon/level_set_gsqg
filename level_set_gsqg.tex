...
\begin{theorem}\label{T1.2}
    ...
    $L(\Omega)<\infty$. Then there is a unique maximal Lagrangian solution
    ...
    $\sup_{t\in J}L(\Phi_{*}^{t}\Omega) < \infty$ for any compact interval $J\subseteq I$.
...
from Theorem~\ref{T1.2}.
    ...
    $\sup_{t\in J}R(\Phi_{*}^{t}\Omega) < \infty$ for each compact interval
...
$\frac{C_{\alpha,n,\beta}}{\max\{\abs{x},\eps\}^{n+2\alpha}}$.
...
\begin{lemma}\label{L2.1}
    ...
    Therefore,
    \[
        \norm{u_\eps(\omega)}_{\dot{C}^{0,1}} \leq C_{\alpha}L(\Omega) \qquad\text{and} \qquad \norm{u(\omega)}_{\dot{C}^{0,1}} \leq C_{\alpha}L(\Omega).
    \]
\end{lemma}
...
%    and the right-hand side of the first equality of the above is $O\left(\abs{h}^{2}\right)$.
    ...
    \end{align*}
   This proves the first claim, and the second follows by
    taking 
...
\begin{lemma}\label{L2.2}
    ...
    \[
        ...
        C_{\alpha} \left(L(\Phi_{1*}\Omega) + L(\Phi_{2*}\Omega)\right)
...
And clearly
    ...
    \]
    so again $\abs{I_{6}} \leq C_{\alpha}L(\Phi_{1*}\Omega)d$.
    ...
    $\abs{I_{2}} \leq C_{\alpha}L(\Phi_{1*}\Omega)d$.  This and \eqref{2.100} now hold uniformly in $x\in\bbR^{2}$, finishing the proof.
\end{proof}
...
$(\mathcal{L},\theta,\Omega)$ with $L(\Omega) < \infty$.
...
is a homeomorphism.  Then so is  $\Phi_{\eps}^{t} = \mathrm{Id} + \Theta_{\eps}^{0,t}$, and so $\omega_{\eps}^{t}\in L^1(\bbR^2)$.
...
\begin{proof}
Letting $x':=x + \Theta_{\eps}^{t_{0},t_{2}}(x)$, we see that 
\[
\Theta_{\eps}^{t_{0},t_{1}}(x) = x' + \Theta_{\eps}^{t_{2},t_{1}}(x')-x=\Theta_{\eps}^{t_{2},t_{1}}(x') + \Theta_{\eps}^{t_{0},t_{2}}(x).
\]
So with $y':=y + \Theta_{\eps}^{t_{0},t_{2}}(y)$, the left-hand side of \eqref{2.101} is just $|\Theta_{\eps}^{t_{2},t_{1}}(x')-\Theta_{\eps}^{t_{2},t_{1}}(y')|$, while the last factor is $|x'-y'|$.  The result now follows from the definition of $\Theta_{\eps}^{t_{2},t}$.
%
...
\begin{corollary}\label{C2.7}
    ...
    \]
    In particular, for all  $t\in(- \frac{1}{C_{\alpha}L(\Omega)},  \frac{1}{C_{\alpha}L(\Omega)})$ we have
    \[
        L(\Phi_{\eps*}^{t}\Omega)
...
\begin{proof}
    ...
    \[
        ...
        \in [L(\Omega), 2L(\Omega)] .
    \]
    We may assume that $L(\Omega) > 0$ because otherwise $\omega^{0} \equiv 0$
...
%    and $t\leq 0$, 
   ...
   And since each $\Phi_{\eps}^{t}$ is measure-preserving,
    ...
    It remains to show that $\Phi^{t}$ satisfies \eqref{1.6}, that is,     with $\omega^{t}\coloneqq\omega^{0}\circ (\Phi^{t})^{-1}$ we have
    \begin{equation}\label{2.7}
        \Phi^{t}(x) = x + \int_{0}^{t}
        u(\omega^{\tau};\Phi^{\tau}(x))\,d\tau
    \end{equation}
for each $t\in[-T_{0},T_{0}]$ and $x\in\bbR^{2}$.
