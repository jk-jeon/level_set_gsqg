...
lower semicontinuity of the perimeter functional (in the sense of Caccioppoli)
with respect to this topology shows measurability of $\lambda'\mapsto\ell(z^{\lambda'})$;
see \cite[Proposition 3.38]{AFPBV} for lower semicontinuity, and see
...
and  flow map $\Phi\in C\left(I;C(\bbR^{2};\bbR^{2})\right)$, and consider $T_{0}\coloneqq \frac{1}{2C_{\alpha}L_{\tht}(\Omega)}$ as in Proposition \ref{P2.8} (note that $[-T_0,T_0]\subseteq I$ because $I$ is maximal). Then since  $\Phi^{t}$ is a homeomorphism for each $t\in I$,  it follows that $\Phi_{*}^{t}\Omega$ is  composed of simple closed curves and we denote these  $z^{t,\lambda}:=\Phi^{t}\circ z^{0,\lambda} \in\operatorname{PSC}(\bbR^{2})$, where $\Phi^{t}\circ z^{0,\lambda}\in \operatorname{CC}(\bbR^{2})$ is the curve whose representative is $\Phi^{t}\circ\tilde{z}^{0,\lambda}$ whenever  $\tilde{z}^{0,\lambda}$ is a representative of $z^{0,\lambda}$ (since $\set{z^{t,\lambda}}_{t\in I}$ is clearly a connected subset of $\operatorname{CC}(\bbR^{2})$,  \cite[Lemma~B.4]{JeoZlaTouching} shows that each $z^{t,\lambda}$ is positively oriented).
...
Our proof that $Q(\Phi_{*}^{t}\Omega) < \infty$ for all small enough $t$ will use the following
...
We now turn to showing that $Q(\Phi_{*}^{t}\Omega) < \infty$ for all small enough $t$.
Since $\ell(\cdot)$ and $\norm{\cdot}_{\dot{H}^{2}}$ are both
lower semicontinuous on $\operatorname{CC}(\bbR^{2})$
...
\end{equation}
for any $t$ with $\abs{t} < T_{1}\coloneqq
...
we now show that $z_{\eps,N}$ converges to $z_{\eps}$ when $N\to\infty$.

\begin{lemma}\label{L3.8}
    Let $T_{0}$ be as in Proposition~\ref{P2.8}. Then
    \[
...
\begin{proof}
    ...
    \eqref{1.7} and Proposition~\ref{P2.8} show
    \[
        M\coloneqq \sup_{(t,N)\in[-T_{0},T_{0}]\times\bbN}
        \max\set{L_{\theta}(\Phi_{\eps,N*}^{t}\omega_{N}^{0}),
        L_{\theta}(\Phi_{\eps*}^{t}\omega_{N}^{0})}
        \leq e^{2\alpha}L_{\theta}(\Omega_{N}) \leq 3L_{\theta}(\Omega).
    \]
    Then for any $t\in[-T_{0},T_{0}]$, Lemmas \ref{L2.1} and \ref{L2.2} show that
    ...
    for some $C_{\alpha}$.
    Since $\Omega$ is of finite measure (with respect to the Lebesgue measure on $\bbR^{2}$
    and $\abs{\theta}$), we clearly have $\omega_{N}^{0} \to \omega^{0}$ in $L^{1}$.
    Therefore, for any $\eta>0$, there is $N_{0}\in\bbN$ independent of $t$ such that
    \begin{align*}
        \norm{u_{\eps}(\Phi_{\eps*}^{t}\omega_{N}^{0})
        - u_{\eps}(\Phi_{\eps*}^{t}\omega^{0})}_{L^{\infty}}
    ...
    \end{align*}
    holds whenever $N\geq N_{0}$. Fix such $N$, then since
    $\norm{\Phi_{\eps,N}^{t}-\Phi_{\eps}^{t}}_{L^{\infty}}$
    ...
    \]
    Hence, a Gr\"{o}nwall-type argument shows
    \[
        \norm{\Phi_{\eps,N}^{t} - \Phi_{\eps}^{t}}_{L^{\infty}}
        \leq \begin{cases}
        ...
        \end{cases}
    \]
    for any $t\in[-T_{0},T_{0}]$, from which the claim follows.
\end{proof}

Since $\mathcal{L} = \bigcup_{N=1}^{\infty}\mathcal{L}_{N}$,
Lemma~\ref{L3.8} shows $z_{\eps,N}^{t,\lambda}\to z_{\eps}^{t,\lambda}$ as $N\to\infty$
for any $(t,\lambda)\in[-T_{0},T_{0}]\times\mathcal{L}$.
By lower semicontinuity of $\ell(\cdot)$ and $\norm{\cdot}_{\dot{H}^{2}}$
(see the paragraph before Lemma~\ref{L3.5}), this and \eqref{3.14} together show that
\[
    \ell(z_{\eps}^{t,\lambda})\norm{z_{\eps}^{t,\lambda}}_{\dot{H}^{2}}^{2}
    \leq \frac{Q(\Omega)}{1 - C_{\alpha}(L_{\theta}(\Omega) + R_{\theta}(\Omega))Q(\Omega)\abs{t}}
\]
for any $(t,\lambda)\in[-T_{1},T_{1}]\times\mathcal{L}$
(recall $T_{1}\coloneqq \frac{1}{C_{\alpha}(L_{\tht}(\Omega) + R_{\tht}(\Omega))Q(\Omega)}$
and $T_{1}\leq T_{0}$ since $Q(\Omega)\geq 4$).
Then again by lower semicontinuity, letting $\eps\to 0^{+}$ shows that
the same inequality holds with $z^{t,\lambda}$ in place of $z_{\eps}^{t,\lambda}$,
so taking the supremum over $\lambda$ finally establishes
\[
    Q(\Phi_{*}^{t}\Omega)
    \leq \frac{Q(\Omega)}{1 - C_{\alpha}(L_{\theta}(\Omega) + R_{\theta}(\Omega))Q(\Omega)\abs{t}}
\]
for $t\in[-T_{1},T_{1}]$. This completes the proof of Theorem~\ref{T1.4}.
