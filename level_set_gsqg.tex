...
In this reason, we call a choice of $(\mathcal{L},\theta,\Omega)$, or simply $\Omega$,
...
\begin{equation}\label{1.4}
    L(\Omega)\coloneqq \sup_{x\in\bbR^{2}}\int_{\mathcal{L}}
    \frac{d|\theta|(\lambda)}{d(x,\partial\Omega^{\lambda})^{2\alpha}} < \infty
\end{equation}
where $\abs{\theta}$ is the variation of $\theta$.
...
\[
    ...
    \}
\]
in $(x,\lambda)$, so $L(\Omega)$ is always well-defined
(with the convention $\frac{1}{0} = \infty$ and $\infty\cdot 0 = 0$).
The quantity $L(\Omega)$ arises in the context of estimating the growth of
...
\begin{definition}
    ...
    \left\{
        \begin{aligned}
           \partial_{t}\Phi^{t}(x) &= u(\omega^{t}; \Phi^{t}(x)), \\
            \Phi^{0}(x) &= x 
        \end{aligned}
    \right.
    \end{equation}
    for all $x\in\bbR^{2}$ where the time derivative is one-sided at any end-point of $I$,
    and each $\Phi^{t}$ is a measure-preserving homeomorphism.
    We call $\Phi$ the \emph{flow map} associdated to $\omega$.
\end{definition}

\textit{Remark}. It is easy to show that any (transport?) solution $\omega$ is
...
\begin{equation}\label{1.7}
    \begin{aligned}
        ...
        \leq \norm{\Phi^{-1}}_{\dot{C}^{0,1}}^{2\alpha} L(\Omega).
    \end{aligned}
\end{equation}
...
\begin{theorem}\label{T1.2}
    ...
    $L(\Omega)<\infty$. Then there is a unique maximal (transport?) solution
    $\omega\colon I\to L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$ to
    \eqref{1.1}--\eqref{1.2} with the initial data $\omega^{0}$
    and the associated flow map $\Phi\in C\left(I;C(\bbR^{2};\bbR^{2})\right)$ such that
    $\sup_{t\in J}L(\Phi_{*}^{t}\Omega) < \infty$ for all compact subinterval $J\subseteq I$.
    Also, $\sup_{t\in J}\max\set{\norm{\Phi^{t}}_{\dot{C}^{0,1}},
    \norm{(\Phi^{t})^{-1}}_{\dot{C}^{0,1}}}<\infty$
    holds for all compact subinterval $J\subseteq I$.
    Furthermore, $I$ is open and if $T$ is an endpoint of $I$, then either $T$ is infinite or
    $\lim_{t\to T}L(\Phi_{*}^{t}\Omega) = \infty$.
\end{theorem}
...
\begin{definition}
    ...
    \begin{equation*}
        ...
        R(\Omega) \coloneqq \sup_{\lambda\in\mathcal{L}}
...
For our second result, we will impose in addition to $L(\Omega)<\infty$
that $\Omega$ is composed of simple closed curves and
$Q(\Omega), R(\Omega) < \infty$. The condition $Q(\Omega)<\infty$ ensures
...
The condition $R(\Omega)<\infty$ on the other hand controls how densely
...
\begin{theorem}\label{T1.4}
    ...
    composed of simple closed curves such that $L(\Omega), Q(\Omega), R(\Omega) < \infty$.
    Let $\omega\colon I\to L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$ be
    the (transport?) solution to \eqref{1.1}--\eqref{1.2}
    with the initial data $\omega^{0}$ and the associated flow map
    $\Phi\in C\left(I;C(\bbR^{2};\bbR^{2})\right)$ provided by Theorem~\ref{T1.2}.
    Then $\Phi_{*}^{t}\Omega$ is composed of simple closed curves for each $t\in I$,
    $\sup_{t\in J}R(\Phi_{*}^{t}\Omega) < \infty$ for each compact subinterval
...
Note that for any $n\in\bbZ_{\geq 0}$ and $\beta\geq 0$ there is $C_{\alpha,n,\beta}$
that only depends on $\alpha,n,\beta$ such that the norm of the $n$-linear form
$D^{n}K_{\eps}(x)$ is bounded by
$\frac{C_{\alpha,n,\beta}}{\eps^{\beta}\abs{x}^{n-\beta+2\alpha}}$.
...
in terms of $L(\Omega)$, where $(\mathcal{L},\theta,\Omega)$ is
a generalized layer cake representation of $\omega$.
Unless specified otherwise, all constants $C_{\alpha}$ in the proofs
can change from one inequality to another and depend only on $\alpha$.

\begin{lemma}\label{L2.1}
    There is $C_{\alpha}$ such that for any
    $\omega\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$
    with a generalized layer cake representation $(\mathcal{L},\theta,\Omega)$,
    $\eps>0$ and $x\in\bbR^{2}$,
    \[
        \abs{D(u_{\eps}(\omega))(x)} \leq
        C_{\alpha}\int_{\mathcal{L}}\frac{d|\theta|(\lambda)}
        {d(x,\partial\Omega^{\lambda})^{2\alpha}}.
    \]
    Therefore, with this $C_{\alpha}$,
    \[
        \norm{u(\omega)}_{\dot{C}^{0,1}} \leq C_{\alpha}L(\Omega)
    \]
    holds for any such $\omega$.
\end{lemma}
...
\begin{proof}
    ...
    Note that whenever $\mathbbm{1}_{\Omega^{\lambda}}(y)
    - \mathbbm{1}_{\Omega^{\lambda}}(x) \neq 0$,
    $\abs{x - y} \geq d(x,\partial\Omega^{\lambda})$ must hold, which shows
    \begin{align*}
    ...
    \end{align*}
    thus the first claim is shown. Then the second claim follows by
    taking the supremum over $x\in\bbR^{2}$ and letting $\eps\to 0^{+}$.
\end{proof}

\begin{lemma}\label{L2.2}
    There is $C_{\alpha}$ such that for any
    $\omega\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$
    with a generalized layer cake representation $(\mathcal{L},\theta,\Omega)$ and
    measure-preserving homeomorphisms\ $\Phi_{1},\Phi_{2}\colon\bbR^{2}\to\bbR^{2}$,
    \[
        \norm{u(\Phi_{1*}\omega) - u(\Phi_{2*}\omega)}_{L^{\infty}} \leq
        C_{\alpha}(L(\Phi_{1*}\Omega) + L(\Phi_{2*}\Omega))
...
\begin{proof}
    ...
    $\omega_{i}\coloneqq \omega\circ\Phi_{i}^{-1}$ for $i=1,2$.
    Then for given $x\in\bbR^{2}$, $u(\Phi_{1*}\omega;x) - u(\Phi_{2*}\omega;x)$
    is equal to the sum of the following two:
    \begin{align*}
    ...
    \[
        ...
        {\abs{x - y}^{1+2\alpha}}\,dy\,d\lambda.
    \]
    In the same way as in the proof of Lemma~\ref{L2.1},
    $\abs{x - y} \geq d(x,\partial\Phi_{1}(\Omega^{\lambda}))$
    must hold whenever the numerator of the integrand in the right-hand side is nonzero,
    which shows
    \begin{align*}
        I_{3} &\leq \int_{\mathcal{L}}\int_{\abs{x - y}\leq 2d}
        \frac{1}{\abs{x - y}\cdot d(x,\partial\Phi_{1}(\Omega^{\lambda}))^{2\alpha}}
        \,dy\,d|\theta|(\lambda)
        \leq 4\pi L(\Phi_{1*}\Omega)d.
    \end{align*}
    Then repeating the same argument for $\omega_{2}$ yields
    \[
        \abs{I_{1}}\leq C_{\alpha}(L(\Phi_{1*}\Omega) + L(\Phi_{2*}\Omega))d.
    \]
    ...
    \[
        ...
        \nabla^{\perp}K(x-y)(\omega_{1}(y) - \omega_{2}(y))\,dy,
    \]
    then $I_{2} = \lim_{R\to\infty}I_{2}^{R}$. Fix $R>2d$, then
    \begin{align*}
        ...
        \nabla^{\perp}K(x - \Phi_{2}(y))(\omega(y) - \omega_{1}(x))\,dy.
    \end{align*}
    Let us call the terms in the right-hand side as $I_{n}$ for $n=4,5,6,7,8$, respectively.

    To estimate $I_{4}$, note that for any $y$ in the domain of integration,
    \[
        \min_{\eta\in[0,1]}
        \abs{x - (1-\eta)\Phi_{1}(y) - \eta\Phi_{2}(y)}
        \geq \abs{x - \Phi_{1}(y)} - d
        \geq \frac{1}{2}\abs{x - \Phi_{1}(y)}
    \]
    holds, so the mean value theorem shows
    \[
        \abs{\nabla^{\perp}K(x - \Phi_{1}(y)) - \nabla^{\perp}K(x - \Phi_{2}(y))}
    ...
    \begin{align*}
        ...
        \leq C_{\alpha}L(\Phi_{1*}\Omega)d.
    \end{align*}

    For $I_{5}$, note that for any $y$ in the domain of integration,
    \[
        \abs{x - \Phi_{1}(y)} \leq \abs{x - \Phi_{2}(y)} + d \leq 3d
    \]
    holds, so applying the change of variables formula yields
    \[
    ...
    shows $\abs{I_{5}} \leq C_{\alpha}L(\Phi_{1*}\Omega)d$.

    Similarly, for $I_{6}$, again the change of variables formula shows
    \[
        \abs{I_{6}} \leq \int_{\abs{x - \Phi_{1}(y)} \leq 2d}
        \frac{C_{\alpha}\abs{\omega(y) - \omega_{1}(x)}}
        {\abs{x - \Phi_{1}(y)}^{1+2\alpha}}\,dy
        \leq \int_{\abs{x - y}\leq 2d}\frac{C_{\alpha}\abs{\omega_{1}(y) - \omega_{1}(x)}}
        {\abs{x - y}^{1+2\alpha}}\,dy,
    \]
    so we obtain $\abs{I_{6}} \leq C_{\alpha}L(\Phi_{1*}\Omega)d$ in the same way
    as in the estimate of $I_{3}$.

    To estimate $I_{7}$, note that for any $y$ in the domain of integration,
    \begin{align*}
    ...
    \begin{align*}
        ...
        \leq \frac{2^{1+2\alpha}\pi C_{\alpha}\norm{\omega}_{L^{\infty}}d}{R^{2\alpha}},
    \end{align*}
    where the last inequality is because $\frac{d}{R} < \frac{1}{2}$.
    In the same way, $I_{8}$ can be estimated by
    $\frac{C_{\alpha}\norm{\omega}_{L^{\infty}}d}{R^{2\alpha}}$.
    
    Therefore, collecting the estimates for
    $I_{4}$ through $I_{8}$ and letting $R\to\infty$ shows
    $\abs{I_{2}} \leq C_{\alpha}L(\Phi_{1*}\Omega)d$.
    Then since $x\in\bbR^{2}$ is arbitrary, collecting
    the estimates for $I_{1}$ and $I_{2}$ shows the desired conclusion.
\end{proof}

\begin{lemma}\label{L2.3}
    There is $C_{\alpha}$ such that for any
    $\omega\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$ and $\eps>0$,
    \[
...
\begin{proof}
    By definition of $K_{\eps}$, we have $\nabla^{\perp}K_{\eps}(x) = \nabla^{\perp}K(x)$
    if $\abs{x}\geq\eps$, so for any $x\in\bbR^{2}$,
    \begin{align*}
...
Now, let initial data $\omega^{0}\in L^{1}(\bbR^{2})\cap L^{\infty}(\bbR^{2})$
admitting a generalized layer cake representation
$(\mathcal{L},\theta,\Omega)$ with $L(\Omega) < \infty$ be given.

Fix $\eps>0$ and consider the ODE
\begin{equation}\label{2.1}
    \left\{
        \begin{aligned}
            \partial_{t}\Psi_{\eps}^{t}(x)
            &= u_{\eps}((\mathrm{Id} + \Psi_{\eps}^{t})_{*}\omega^{0};x + \Psi_{\eps}^{t}(x)), \\
            \Psi_{\eps}^{0}(x) &= 0.
        \end{aligned}
    \right.
\end{equation}
...
$u_{\eps}$ which in turn is generated by the measure $\omega^{0}$
transported by $\Phi_{\eps}^{t}$. We show that this ODE is globally well-posed in
the Banach space $C_{b}(\bbR^{2};\bbR^{2})$
of bounded continuous functions $\bbR^{2}\to\bbR^{2}$.

\begin{lemma}\label{L2.4}
    For each $F\in C_{b}(\bbR^{2};\bbR^{2})$, define
    \[
        \mathcal{F}(F)(x) \coloneqq u_{\eps}((\mathrm{Id} + F)_{*}\omega^{0}; x + F(x))
    \]
    for each $x\in\bbR^{2}$, then $\mathcal{F}\colon
    C_{b}(\bbR^{2};\bbR^{2})\to C_{b}(\bbR^{2};\bbR^{2})$
    is well-defined and is Lipschitz continuous.
\end{lemma}

\begin{proof}
    Clearly, $\mathcal{F}(F)$ is a continuous function $\bbR^{2}\to\bbR^{2}$
    for any $F\in C_{b}(\bbR^{2};\bbR^{2})$.
    Given $F_{1},F_{2}\in C_{b}(\bbR^{2};\bbR^{2})$ and $x\in\bbR^{2}$,
    \begin{equation*}
        \begin{aligned}
            &(\mathcal{F}(F_{1}) - \mathcal{F}(F_{2}))(x)
            \\&\ 
...
Therefore, \eqref{2.1} is globally well-posed.
Then with $\Phi_{\eps}^{t}\coloneqq \mathrm{Id} + \Psi_{\eps}^{t}$ and
$\omega_{\eps}^{t}\coloneqq\Phi_{\eps*}^{t}\omega^{0}$ (which for now we only regard
as a finite signed Borel measure rather than an $L^{1}$ function),
we can now easily see that the ODE
\begin{equation}\label{2.2}
    \partial_{t}G^{t}(x) = u_{\eps}(\omega_{\eps}^{t};x + G^{t}(x))
\end{equation}
is globally well-posed in $C_{b}(\bbR^{2};\bbR^{2})$ for any initial data at any initial time.
For each $t_{0}\in\bbR$, let $\Theta_{\eps}^{t_{0},t}$ be the unique solution to \eqref{2.2}
with the initial data $\Theta_{\eps}^{t_{0},t_{0}} = 0$, and consider
$G^{t} \coloneqq \Theta_{\eps}^{t_{0},t_{1}} +
\Theta_{\eps}^{t_{1},t}\circ(\mathrm{Id} + \Theta_{\eps}^{t_{0},t_{1}})$, then
it can be easily seen that $G$ solves \eqref{2.2} and it satisfies
$G^{t_{1}} = \Theta_{\eps}^{t_{0},t_{1}}$, so the uniqueness of the solution
with the initial data $\Theta_{\eps}^{t_{0},t_{1}}$ at $t=t_{1}$ shows that
\[
...
\]
holds for all $t\in\bbR$. Hence, letting $t=t_{0}$ shows
$(\mathrm{Id} + \Theta_{\eps}^{t_{1},t_{0}})
\circ(\mathrm{Id} + \Theta_{\eps}^{t_{0},t_{1}}) = \mathrm{Id}$,
so we conclude that each $\mathrm{Id} + \Theta_{\eps}^{t_{0},t}$,
and in particular $\Phi_{\eps}^{t} = \mathrm{Id} + \Theta_{\eps}^{0,t}$,
is a homeomorphism.

On the other hand, let $C_{b}^{1}(\bbR^{2};\bbR^{2})$ be the Banach space of
bounded $C^{1}$-functions $\bbR^{2}\to\bbR^{2}$ with bounded derivatives, then
for $F\in C_{b}^{1}(\bbR^{2};\bbR^{2})$ and $x,h\in\bbR^{2}$,
\begin{align*}
...
is locally Lipschitz on $C_{b}^{1}(\bbR^{2};\bbR^{2})$, so
\eqref{2.2} is locally well-posed in $C_{b}^{1}(\bbR^{2};\bbR^{2})$.
In fact it is globally well-posed in that space, because
\[
    \norm{D(u_{\eps}(\omega_{\eps}^{t})\circ(\mathrm{Id} + F))}_{L^{\infty}}
    \leq \norm{D(\nabla^{\perp}K_{\eps})}_{L^{\infty}}\norm{\omega^{0}}_{L^{1}}
    \norm{\mathrm{Id} + DF}_{L^{\infty}}
\]
for any $F\in C_{b}^{1}(\bbR^{2};\bbR^{2})$, which together with
a Gr\"{o}nwall-type argument shows that the $C^{1}$ norm of the solution to \eqref{2.2}
can grow only at most exponentially.
Hence, $\Theta_{\eps}^{t_{0},t}$ is in $C_{b}^{1}(\bbR^{2};\bbR^{2})$
for all $t\in\bbR$, and since the vector field $u_{\eps}(\omega_{\eps}^{t})$
is divergence-free, it follows that each $\mathrm{Id} + \Theta_{\eps}^{t_{0},t}$
is measure-preserving. Therefore, the measure $\omega_{\eps}^{t}$ corresponds to
the $L^{1}$ function $\omega^{0}\circ(\Phi_{\eps}^{t})^{-1}$, of which
$\Phi_{\eps*}^{t}\Omega$ is a generalized layer cake representation.\smallskip

\textit{Remark}. Similarly, let $C_{b}^{2}(\bbR^{2};\bbR^{2})$ be the Banach space of
bounded $C^{2}$-functions $\bbR^{2}\to\bbR^{2}$ with bounded first and second derivatives,
then for $F\in C_{b}^{2}(\bbR^{2};\bbR^{2})$ and $x,h_{1},h_{2}\in\bbR^{2}$,
\begin{align*}
...
\begin{align*}
    ...
    + \norm{D(\nabla^{\perp}K_{\eps})}_{L^{\infty}}\norm{D^{2}F}_{L^{\infty}}
\end{align*}
hold. Therefore, at most exponential growth of the $C^{1}$ norm with
another Gr\"{o}nwall-type argument show that \eqref{2.2} is globally well-posed
in $C_{b}^{2}(\bbR^{2};\bbR^{2})$. This fact will be used in Section~\ref{S3}.
Repeating a similar argument inductively for higher derivatives shows that
\eqref{2.2} is globally well-posed in every $C_{b}^{k}(\bbR^{2};\bbR^{2})$
so each $\Phi_{\eps}^{t}$ is a diffeomorphism, but we do not need that fact.\smallskip

Now, we derive an $\eps$-independent estimate on the growth of
$L(\Phi_{\eps*}^{t}\Omega)$.
...
\begin{proposition}\label{P2.6}
    $L(\Phi_{\eps*}^{t}\Omega)$ is continuous in $t$ and
    \[
        \max\set{
            \partial_{t}^{+}L(\Phi_{\eps*}^{t}\Omega),
            -\partial_{t-}L(\Phi_{\eps*}^{t}\Omega)
        }
        \leq \norm{u_{\eps}(\omega_{\eps}^{t})}_{\dot{C}^{0,1}}
        L(\Phi_{\eps*}^{t}\Omega)
...
\begin{proof}
    Fix $t_{1},t_{2}\in\bbR$, $x\in\bbR^{2}$, $\lambda\in\mathcal{L}$ and $\eta>0$.
    Then there exists $y\in\partial\Omega^{\lambda}$ such that
    \[
        d(\Phi_{\eps}^{t_{1}}(x), \partial\Phi_{\eps}^{t_{1}}(\Omega^{\lambda})) + \eta
        \geq \abs{\Phi_{\eps}^{t_{1}}(x) - \Phi_{\eps}^{t_{1}}(y)}.
    \]
    Then Lemma~\ref{L2.5} and
    the inequality $\abs{\frac{1}{a^{2\alpha}} - \frac{1}{b^{2\alpha}}}
    \leq \frac{\abs{a - b}}{ab^{2\alpha}}$ for $a,b>0$ show
    ...
    and then taking supremum over $x$ shows
    \[
        L(\Phi_{\eps*}^{t_{1}}\Omega)
        \leq \exp\left(\abs{
            \int_{t_{2}}^{t_{1}}
            \norm{u_{\eps}(\omega_{\eps}^{\tau})}_{\dot{C}^{0,1}}\,d\tau
        }\right)L(\Phi_{\eps*}^{t_{2}}\Omega).
    \]
    Since $t_{1},t_{2}\in\bbR$ are arbitrary, all claims follow immediately.
\end{proof}


\begin{corollary}\label{C2.7}
    With $C_{\alpha}$ from Lemma~\ref{L2.1},
    \[
        \max\set{
            \partial_{t}^{+}L(\Phi_{\eps*}^{t}\Omega),
            -\partial_{t-}L(\Phi_{\eps*}^{t}\Omega)
        }
        \leq C_{\alpha}L(\Phi_{\eps*}^{t}\Omega)^{2}
    \]
    holds for any $t\in\bbR$. Also, whenever $\abs{t} < \frac{1}{C_{\alpha}L(\Omega)}$,
    \[
        L(\Phi_{\eps*}^{t}\Omega)
        \leq \frac{L(\Omega)}{1 - C_{\alpha}L(\Omega)\abs{t}}
    \]
    holds.
\end{corollary}

\begin{proof}
    Since the second claim of Lemma~\ref{L2.1} also holds with $u_{\eps}$ in place of $u$,
    Proposition~\ref{P2.6} shows the first claim, and the second claim
    follows from the first claim and a Gr\"{o}nwall-type argument.
\end{proof}

Let $T_{0}\coloneqq \frac{1}{2C_{\alpha}L(\Omega)}$ with $C_{\alpha}$ from Lemma~\ref{L2.1}.

\begin{proposition}\label{P2.8}
    When $\eps\to 0^{+}$,
    $\Psi_{\eps}\in C\left([-T_{0},T_{0}];C_{b}(\bbR^{2};\bbR^{2})\right)$
    converges uniformly to some $\Psi\in C\left([-T_{0},T_{0}];C_{b}(\bbR^{2};\bbR^{2})\right)$.
    Also, $\Phi^{t}\coloneqq\mathrm{Id} + \Psi^{t}$
    is a measure-preserving homeomorphism for each $t\in[-T_{0},T_{0}]$ and
    $\Phi$ solves \eqref{1.6}. Furthermore,
    \[
        L(\Phi_{*}^{t}\Omega) \leq
        \sup_{\eps>0}L(\Phi_{\eps*}^{t}\Omega) \leq 2L(\Omega)
    \]
    and
    \[
        ...
        \leq e^{2C_{\alpha}L(\Omega)\abs{t}}
    \]
    hold for each $t\in[-T_{0},T_{0}]$.
\end{proposition}

\begin{proof}
    Corollary~\ref{C2.7} shows
    \[
        M\coloneqq \sup_{\eps>0}\sup_{t\in[-T_{0},T_{0}]}L(\Phi_{\eps*}^{t}\Omega)
        \in [L(\Omega), 2L(\Omega)] \subseteq [0,\infty).
    \]
    We may assume $L(\Omega) > 0$ because otherwise $\omega^{0} \equiv 0$
    so the conclusion follows trivially.
    Fix $t_{0}\in[-T_{0},T_{0}]$, then for any $t\in[-T_{0},T_{0}]$, $\eps>0$ and
    $\eps'\in(0,\eps)$, Lemmas \ref{L2.1}, \ref{L2.2} and \ref{L2.3} show
    ...
    is two times the maximum of all $C_{\alpha}$'s appearing in those lemmas.
    Then for any $t_{1},t_{2}\in[-T_{0},T_{0}]$,
    integrating \eqref{2.5} from $t_{2}$ to $t_{1}$ shows
    ...
    and letting $t_{1}\to t_{2}^{\pm}$ shows
    \begin{align*}
        \max\set{
        ...
        + C_{\alpha}\norm{\omega^{0}}_{L^{\infty}}\eps^{1-2\alpha}
    \end{align*}
    for each $t\in[-T_{0},T_{0}]$, so a Gr\"{o}nwall-type argument shows
    \[
        \norm{\Phi_{\eps}^{t} - \Phi_{\eps'}^{t}}_{L^{\infty}}
        = \norm{\Psi_{\eps}^{t} - \Psi_{\eps'}^{t}}_{L^{\infty}}
    ...
    \]
    Applying this inequality to \eqref{2.6} with $C_{\alpha}M$'s replaced by $2C_{\alpha}M$'s,
    dividing by $\abs{t_{1} - t_{2}}$ and then sending $t_{1}\to t_{2}^{+}$ shows
    \[
        \partial_{t}^{+}\norm{\Theta_{\eps}^{t_{0},t} - \Theta_{\eps'}^{t_{0},t}}_{L^{\infty}}
        \leq 2C_{\alpha}M\norm{\Theta_{\eps}^{t_{0},t} - \Theta_{\eps'}^{t_{0},t}}_{L^{\infty}}
        + C_{\alpha}\norm{\omega^{0}}_{L^{\infty}}
        e^{2C_{\alpha}Mt}\eps^{1-2\alpha}
    \]
    for $t\geq 0$, and we can also obtain a similar bound for
    $-\partial_{t-}\norm{\Theta_{\eps}^{t_{0},t} - \Theta_{\eps'}^{t_{0},t}}_{L^{\infty}}$
    and $t\leq 0$, so a Gr\"{o}nwall-type argument shows
    \begin{equation*}
        \norm{\Theta_{\eps}^{t_{0},t} - \Theta_{\eps'}^{t_{0},t}}_{L^{\infty}}
        \leq \frac{\norm{\omega^{0}}_{L^{\infty}}e^{2C_{\alpha}MT_{0}}\eps^{1-2\alpha}}{2M}
        \left(e^{2C_{\alpha}M\abs{t - t_{0}}} - 1\right).
    \end{equation*}
    Therefore, $\Theta_{\eps}^{t_{0},\,\cdot\,}$ converges uniformly
    to some $\Theta^{t_{0},\,\cdot\,}\colon [-T_{0},T_{0}]\to C_{b}(\bbR^{2};\bbR^{2})$
    as $\eps\to 0^{+}$. We let $\Psi^{t}\coloneqq\Theta^{0,t}$.

    Since $(\mathrm{Id} + \Theta_{\eps}^{t_{0},t_{1}})
    \circ(\mathrm{Id}+\Theta_{\eps}^{t_{1},t_{0}}) = \mathrm{Id}$ holds for
    all $t_{0},t_{1}\in[-T_{0},T_{0}]$ and $\eps>0$, sending $\eps\to 0^{+}$ shows
    $(\mathrm{Id} + \Theta^{t_{0},t_{1}})
    \circ(\mathrm{Id} + \Theta^{t_{1},t_{0}}) = \mathrm{Id}$, so in particular
    $\Phi^{t}\coloneqq \mathrm{Id} + \Psi^{t}$ is a homeomorphism whose inverse is
    $\mathrm{Id} + \Theta^{t,0}$.
    Also, Lemma~\ref{L2.5} and the definition of $C_{\alpha}$ show
    ...
    By Fatou's lemma, we also have $L(\Phi_{*}^{t}\Omega)
    \leq \liminf_{\eps\to 0^{+}}L(\Phi_{\eps*}^{t}\Omega) \leq M$ for each $t\in[-T_{0},T_{0}]$.
    Furthermore, since each $\Phi_{\eps}^{t}$ is measure-preserving,
    its uniform limit $\Phi^{t}$ is also measure-preserving, because
    $\mathbbm{1}_{U}\circ\Phi_{\eps}^{t}$ converges pointwise to
    $\mathbbm{1}_{U}\circ\Phi^{t}$ as $\eps\to 0^{+}$ for any open set $U\subseteq\bbR^{2}$.

    It remains to show that $\Phi^{t}$ satisfies \eqref{1.6}, or equivalently, that
    with $\omega^{t}\coloneqq\omega^{0}\circ (\Phi^{t})^{-1}$,
    \begin{equation}\label{2.7}
        \Phi^{t}(x) = x + \int_{0}^{t}
        u(\omega^{\tau};\Phi^{\tau}(x))\,d\tau
    \end{equation}
    holds for each $t\in[-T_{0},T_{0}]$ and $x\in\bbR^{2}$.
    Note that taking $t_{0} = 0$ and letting $\eps'\to 0^{+}$ in \eqref{2.5} yields
    \begin{align*}
    ...
    \[
        \Phi_{\eps}^{t}(x) = x + \int_{0}^{t}
        u(\omega_{\eps}^{\tau};\Phi_{\eps}^{\tau}(x))\,d\tau
    \]
    converges to the right-hand side of \eqref{2.7}
    when $\eps\to 0^{+}$, thus proving \eqref{1.6}.
\end{proof}
...
\begin{proposition}\label{P2.9}
    ...
    $\sup_{t\in I}\max\set{L(\Phi_{*}^{t}\Omega),L(\Pi_{*}^{t}\Omega)}<\infty$.
    Then $\Phi = \Pi$.
\end{proposition}
...
\begin{proof}
    ...
    \[
        ...
        \right\}.
    \]
    Then Lemmas \ref{L2.1} and \ref{L2.2} show
    \begin{align*}
        ...
        &\leq C_{\alpha}M\norm{\Phi^{t} - \Pi^{t}}_{L^{\infty}}
    \end{align*}
    for some $C_{\alpha}$, which together with continuity of
    ...
    \[
        ...
        \leq C_{\alpha}M\norm{\Phi^{t} - \Pi^{t}}_{L^{\infty}}.
    \]
    Therefore, a Gr\"{o}nwall-type argument proves the claim.
\end{proof}

Combining Propositions \ref{P2.8} and \ref{P2.9} then shows Theorem~\ref{T1.2}.
...
(transport?) solution to \eqref{1.1}--\eqref{1.2} with the initial data $\omega^{0}$
...
in $C_{b}^{2}(\bbR^{2};\bbR^{2})$ (see the remark before Lemma~\ref{L2.5}),
...
\begin{corollary}\label{C3.2}
    ...
    \[
        e^{-3C_{\alpha}L(\Phi_{*}^{t}\Omega)\abs{h}}\ell(z^{t,\lambda})
...
\begin{proof}
    ...
    \begin{equation}\label{3.3}
        ...
        \leq \norm{u_{\eps}(\omega_{\eps}^{t})}_{\dot{C}^{0,1}}
        \ell(z_{\eps}^{t,\lambda})
        \leq 2C_{\alpha}L(\Omega)\ell(z_{\eps}^{t,\lambda})
    \end{equation}
    for any $t\in\bbR$ with $\abs{t}\leq\frac{1}{2C_{\alpha}L(\Omega)}$
    ...
    \begin{equation}\label{3.4}
        \ell(z^{t,\lambda}) \leq e^{2C_{\alpha}L(\Omega)\abs{t}}\ell(z^{0,\lambda})
    \end{equation}
    holds for such $t$.

    For any $t_{0}\in I$ with $\abs{t_{0}}\leq\frac{1}{4C_{\alpha}L(\Omega)}$,
    ...
    for $t\in I$ with $\abs{t - t_{0}}\leq\frac{1}{2C_{\alpha}L(\Phi_{*}^{t_{0}}\Omega)}$.
    ...
    Since $L(\Phi_{*}^{t_{0}}\Omega) \leq \frac{4}{3}L(\Omega)$ by Corollary~\ref{C2.7},
    we can substitute $0$ to $t$ which shows
    \[
        \ell(z^{0,\lambda}) \leq e^{3C_{\alpha}L(\Omega)\abs{t_{0}}}\ell(z^{t_{0},\lambda}).
    \]
    Therefore,
    \[
        e^{-3C_{\alpha}L(\Omega)\abs{t}}\ell(z^{0,\lambda})
        \leq \ell(z^{t,\lambda}) \leq
        e^{3C_{\alpha}L(\Omega)\abs{t}}\ell(z^{0,\lambda})
    \]
    holds for any $t\in\bbR$ with $\abs{t}\leq\frac{1}{4C_{\alpha}L(\Omega)}$.
    Applying this inequality to $(\omega^{t},t)$ in place of $(\omega^{0},0)$
    for $t\in I$ then shows the claim.
\end{proof}

\begin{proposition}\label{P3.3}
    $R(\Phi_{*}^{t}\Omega)$ is continuous in $t$, so in particular
    for any compact subinterval $J\subseteq I$,
    \[
        \sup_{t\in J}R(\Phi_{*}^{t}\Omega) < \infty.
    \]
\end{proposition}
...
\begin{proof}
    ...
    \begin{equation}\label{3.5}
        ...
        - \Phi_{\eps}^{t_{2}}(x) + \Phi_{\eps}^{t_{2}}(y)}
        \leq (e^{2C_{\alpha}L(\Omega)\abs{t_{1}-t_{2}}} - 1)
        \abs{\Phi_{\eps}^{t_{1}}(x) - \Phi_{\eps}^{t_{1}}(y)}
    \end{equation}
    ...
    \[
        M\coloneqq \sup_{t\in J}L(\Phi_{t*}\Omega) < \infty.
    \]
    ...
    \begin{equation}\label{3.9}
        R(\Phi_{*}^{t_{1}}\Omega)
        \leq e^{5C_{\alpha}M\abs{t_{1} - t_{2}}}R(\Phi_{*}^{t_{2}}\Omega).
    \end{equation}
...
$R(\Phi_{\eps*}^{t}\Omega)$, we first state a relevant bound on it
which we already have obtained in the proof of Proposition~\ref{P3.3}.

\begin{lemma}\label{L3.4}
    $R(\Phi_{\eps*}^{t}\Omega)$ is continuous in $t$, and with
    $C_{\alpha}$ from Lemma~\ref{L2.1} and $T_{0}$ from Proposition~\ref{P2.8},
    \begin{equation}\label{3.10}
        R(\Phi_{\eps*}^{t}\Omega) \leq e^{4C_{\alpha}L(\Omega)\abs{t}}R(\Omega)
        \leq 8R(\Omega)
    \end{equation}
...
\begin{proof}
    ...
    \[
        e^{-2C_{\alpha}L(\Omega)\abs{t_{1} - t_{2}}}\ell(z_{\eps}^{t_{2},\lambda})
    ...
    \[
        ...
        \right)^{2\alpha}}
        \leq \frac{e^{2C_{\alpha}L(\Omega)\abs{t_{1} - t_{2}}}}
    ...
    $(z_{\eps},4C_{\alpha}L(\Omega),\Phi_{\eps})$ in place of $(z,5C_{\alpha}M,\Phi)$,
...
\begin{lemma}\label{L3.6}
    ...
    such that $L(\Omega)<\infty$, $\eps>0$ and $x,h_{1},h_{2}\in\bbR^{2}$,
    \begin{align*}
...
\begin{lemma}\label{L3.7}
    ...
    \begin{equation}\label{3.11}
        \abs{\partial_{t}\norm{z_{\eps}^{t,\lambda}}_{\dot{H}^{2}}^{2}}
        \leq C_{\alpha}(L(\Phi_{\eps*}^{t}\Omega) + R(\Phi_{\eps*}^{t}\Omega))
        Q(\Phi_{\eps*}^{t}\Omega)\norm{z_{\eps}^{t,\lambda}}_{\dot{H}^{2}}^{2}
    \end{equation}
\end{lemma}
...
\begin{proof}
    ...
    \[
        5\norm{u_{\eps}(\omega_{\eps}^{t})}_{\dot{C}^{0,1}}
        \norm{z_{\eps}^{t,\lambda}}_{\dot{H}^{2}}^{2}
        \leq 5C_{\alpha}L(\Phi_{\eps*}^{t}\Omega)\norm{z_{\eps}^{t,\lambda}}_{\dot{H}^{2}}^{2}
        \leq 5C_{\alpha}L(\Phi_{\eps*}^{t}\Omega)
        Q(\Phi_{\eps*}^{t}\Omega)\norm{z_{\eps}^{t,\lambda}}_{\dot{H}^{2}}^{2}
    \]
    ...
    Since $L(\Phi_{\eps*}\Omega)<\infty$, Lemma~\ref{L3.6} and Green's theorem show that
    \begin{align*}
    ...
    Therefore, we obtain
    \begin{align*}
        \abs{G_{2}} \leq C_{\alpha}R(\Phi_{\eps*}\Omega)
        Q(\Phi_{\eps*}\Omega)\norm{z_{\eps}^{\lambda}}_{\dot{H}^{2}}^{2}.
    \end{align*}

    \textbf{Estimate for $G_{3}$.} We can assume $R(\Phi_{\eps*}\Omega)<\infty$
    ...
    $C_{\alpha}R(\Phi_{\eps*}\Omega)Q(\Phi_{\eps*}\Omega)
    ...
    \begin{align*}
        ...
        \,ds\,d|\theta|(\lambda')
        \\&\quad\quad \leq
        C_{\alpha}L(\Phi_{\eps*}\Omega)Q(\Phi_{\eps*}\Omega)
        \int_{\ell^{\lambda}\bbT}
        \abs{\kappa^{\lambda}(s)}\mathcal{M}\kappa^{\lambda}(s)\,ds
        \leq C_{\alpha}L(\Phi_{\eps*}\Omega)Q(\Phi_{\eps*}\Omega)
        \norm{z_{\eps}^{\lambda}}_{\dot{H}^{2}}^{2},
    \end{align*}
...
by the isoperimetric inequality. Clearly, $L(\Omega_{N}) \leq L(\Omega) < \infty$.
...
and the inequalities $L(\Omega_{N})\leq L(\Omega)$,
$R(\Omega_{N})\leq R(\Omega)$, and $Q(\Phi_{\eps,N*}^{t}\Omega) \geq 4$, we see that
\begin{align*}
    ...
    (L(\Omega) + R(\Omega))Q(\Phi_{\eps,N*}^{\tau}\Omega)^{2}\,d\tau}
\end{align*}
...
\[
    \max\set{
        ...
        -\partial_{t-}Q(\Phi_{\eps,N*}^{t}\Omega)
    } \leq C_{\alpha}(L(\Omega) + R(\Omega))Q(\Phi_{\eps,N*}^{t}\Omega)^{2},
\]
which we then use with a Gr\"{o}nwall-type argument to conclude
\begin{equation}\label{3.14}
    Q(\Phi_{\eps,N*}^{t}\Omega) \leq \frac{Q(\Omega_{N})}
    {1 - C_{\alpha}(L(\Omega) + R(\Omega))Q(\Omega_{N})\abs{t}}
    \leq \frac{Q(\Omega)}
    {1 - C_{\alpha}(L(\Omega) + R(\Omega))Q(\Omega)\abs{t}}
\end{equation}
for any $t$ with $\abs{t} < \frac{1}{C_{\alpha}(L(\Omega) + R(\Omega))Q(\Omega)}$,
...
\begin{proof}[Proof of Lemma~\ref{L3.8}]
    ...
    First, note that the inequality $L(\Omega_{N})\leq L(\Omega)$
    ...
    \eqref{3.15} and the inequality $L(\Omega_{N}) \leq L(\Omega)$ show
    \begin{align*}
        \abs{V_{1}} &\leq \norm{u_{\eps}(\omega_{\eps,N}^{t})}_{\dot{C}^{0,1}}
        \abs{D\Phi_{\eps,N}^{t}(x) - D\Phi_{\eps}^{t}(x)}
        \leq C_{\alpha}L(\Phi_{\eps,N*}^{t}\Omega_{N})
        \abs{D\Phi_{\eps,N}^{t}(x) - D\Phi_{\eps}^{t}(x)} \\
        &\leq C_{\alpha}L(\Omega)\norm{\left.\left(
    ...
    \begin{align*}
        ...
        &\leq \frac{C_{\alpha}}{\eps}\norm{\Phi_{\eps,N}^{t}}_{\dot{C}^{0,1}}
        \norm{(\Phi_{\eps,N}^{t})^{-1}}_{\dot{C}^{0,1}}^{2\alpha}L(\Omega) \\
        &\leq \frac{C_{\alpha}}{\eps}L(\Omega).
    \end{align*}
    ...
    \begin{align*}
        \abs{\partial_{t}\left(
            D\Phi_{\eps,N}^{t}(x) - D\Phi_{\eps}^{t}(x)
        \right)} &\leq C_{\alpha,\eps,L(\Omega),\norm{\omega^{0}}_{L^{\infty}}}
        \norm{\left.\left(D\Phi_{\eps,N}^{t}
        - D\Phi_{\eps}^{t}\right)\right|_{B_{R_{\delta}}(x_{0})}}_{L^{\infty}}
        + C_{\alpha,\eps}\delta
    \end{align*}
    ...
    \[
        \norm{\left.\left(D\Phi_{\eps,N}^{t}
        - D\Phi_{\eps}^{t}\right)\right|_{B_{R_{\delta}}(x_{0})}}_{L^{\infty}}
        \leq \frac{\exp(C_{\alpha,\eps,L(\Omega),\norm{\omega^{0}}_{L^{\infty}}}\abs{t}) - 1}
        {C_{\alpha,\eps,L(\Omega),\norm{\omega^{0}}_{L^{\infty}}}}
        C_{\alpha,\eps}\delta
    \]
...
\section{Regularity of Functions with $L(\Omega)<\infty$}
...
\begin{proposition}
    ...
    \[
        \norm{\omega}_{\dot{C}^{0,2\alpha}} \leq L(\Omega)
    \]
    holds.
\end{proposition}
...
\begin{proof}
    ...
    \[
        ...
        {d(x,\partial\Omega^{\lambda})^{2\alpha}}\,d|\theta|(\lambda)
        \leq L(\Omega)\abs{x - y}^{2\alpha},
    \]
    thus $\norm{\omega}_{\dot{C}^{0,2\alpha}} \leq L(\Omega)$ follows.
\end{proof}
...
\begin{lemma}
    ...
    $(\mathcal{L},\theta,\Omega)$ of $\omega^{0}$,
    $L(\Omega) \leq 2^{2\alpha}\hat{L}(\Omega)$ holds.
\end{lemma}
...
\begin{proof}
    ...
    we obtain $L(\Omega)\leq 2^{2\alpha}\hat{L}(\Omega)$.
\end{proof}