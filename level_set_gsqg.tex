...
%    All constants $C_{\alpha}$ in this proof depend only on $\alpha$ and
    ...
    \begin{equation}\label{3.100}
        ...
        \,ds'\,ds\,d|\theta|(\lambda'),
    \end{equation}
    where $\mathcal{M}$ is the maximal operator
    \begin{equation*}
    ...
    \end{equation*}
    for $f\in L^{1}(\ell\bbT)$ and $\ell\in(0,\infty)$,
    from (A.2) in \cite{JeoZlaTouching}.
    Let us split the integrand of \eqref{3.100} into the sum of two terms with numerators
    $\abs{\kappa_{\eps}^{\lambda}(s)}\mathcal{M}\kappa_{\eps}^{\lambda}(s)$ and
    $\abs{\kappa_{\eps}^{\lambda}(s)}\mathcal{M}\kappa_{\eps}^{\lambda'}(s')$, respectively.  Then
    the maximal inequality (see (A.3) in \cite{JeoZlaTouching})
    \begin{equation*}
        \norm{\mathcal{M}f}_{L^{2}} \leq C\norm{f}_{L^{2}}
    \end{equation*}
    for $f\in L^{2}(\ell\bbT)$ and $\ell\in(0,\infty)$
    with a universal constant $C$ shows that  the same argument
